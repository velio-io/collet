...
(deftest ^:integration jdbc-make-query-test
  (let [pg             (containers/postgres)
    ...
    (with-open [conn (jdbc/get-connection connection-map)]
      (containers/populate-users! conn))
...
(deftest ^:integration jdbc-execute-action-test
  (let [pg             (containers/postgres)
        port           (get (:mapped-ports pg) 5432)
        connection-map {:dbtype   "postgresql"
                        :host     "localhost"
...
(deftest ^:integration postgres-various-data-types-query-test
  (let [pg             (containers/postgres)
        port           (get (:mapped-ports pg) 5432)
        connection-map {:dbtype   "postgresql"
                        :host     "localhost"
...
(deftest ^:integration execute-pipeline-with-jdbc-action
  (let [mysql          (containers/mysql)
...
(deftest ^:integration join-tables-query-test
  (let [mysql          (containers/mysql)
        port           (get (:mapped-ports mysql) 3306)
        connection-map {:dbtype   "mysql"
                        :host     "localhost"
