  ...
  (testing "joining a sequence with another sequence"
    (let [result (sut/prep-dataset
                  {:sequence [{:id 1 :name "John"}
                              ...
                              {:id 3 :name "Jack"}]
                   :apply    [[:join
                               {:with   [{:user {:id 1} :city "Springfield"}
  ...
  (testing "select from sequence"
    ...
    (let [data-seq [[{:id 1 :name "John" :city "Springfield"}
      ...
      (is (= [{:id 2 :city "Lakeside"}
              {:id 3 :city "Roanoke"}]
             (->> (ds/rows (first result))
                  (map #(update % :city str))))
          "first dataset")
      (is (= [{:id 9 :city "Florence"}
              {:id 10 :city "Florence"}]
             (->> (ds/rows (second result))
                  (map #(update % :city str))))
          "second dataset")
...
(def test-events-data
  [{:id        "9a624d46-8cca-43bf-a58a-f8f21a2f98d8"
   ...
   {:id        "807f3f14-f3b0-41ff-959a-82bfae11b3a9"
    ...
    :relations [{:type "support act" :type-id "492a850e-97eb-306a-a85e-4b6d98527796" :direction "backward" :artist {:id "bc4e41d2-da66-415b-a57f-8f76dbed1729" :name "Jacques Tellitocci" :sort-name "Tellitocci, Jacques"}}
...
(deftest fold-multiple-columns-test
  (let
    [pulls [{:id 1 :user-id 1 :closed_at "2025-02-19"}
